...
export default function EditChallengePage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    async function loadChallenge() {
      ...
      if (json.challenge) {
        ...
        setLanguages(c.supportedLanguages?.map((l: any) => l.name) || ["javascript"])
        setTestCases(c.testCases || [{ input: "", output: "" }])
        
        const codeMap: Record<string, string> = {}
        c.starterCode?.forEach((s: any) => {
          codeMap[s.language] = s.code
        })
        setStarterCode(codeMap)
      }
      setLoading(false)
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!user) {
      alert("Please sign in to edit challenges")
      return
    }
    ...
    const starterCodeArray = Object.entries(starterCode).map(([language, code]) => ({
      language,
      code
    }))
    ...
    if (json.ok) {
      ...
      alert(json.error || "Error updating challenge")
    }
  }
    ...
    <section className="container mx-auto px-6 py-10">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          <p className="text-sm uppercase tracking-widest text-slate-500">Edit</p>
          <h2 className="text-3xl font-semibold text-slate-900">Edit Challenge</h2>
        </div>
        <form onSubmit={handleSubmit} className="glass-panel rounded-2xl p-8 space-y-6">
          ...
          <div>
            <label className="block mb-2 text-sm font-semibold text-slate-600">
              Supported Languages (Ctrl/Cmd+click to select multiple)
            </label>
            <select
              multiple
              value={languages}
              onChange={(e) =>
                setLanguages(Array.from(e.target.selectedOptions, (o) => o.value))
              }
          ...
          <div className="flex gap-4">
            <button
              className="flex-1 rounded-full bg-gradient-to-r from-indigo-500 via-violet-500 to-cyan-500 px-6 py-3 text-sm font-semibold text-white shadow-glow transition duration-300 ease-swift-out hover:-translate-y-0.5"
              type="submit"
            >
              Update Challenge
            </button>
            <button
              ...
              className="rounded-full border border-slate-200 px-6 py-3 text-sm font-semibold text-slate-600 transition duration-300 ease-swift-out hover:bg-slate-50 hover:-translate-y-0.5"
            >
              Cancel
            </button>
          </div>
